...
class RunBuildDependenciesTaskBuilderTest extends AbstractProjectBuilderSpec {
    ...
    def "closed project build dependencies are executed on build"() {
        ...
        then:
        def buildTask = project.tasks.getByName("eclipseClosedProjectBuildDependencies")
        buildTask != null
        buildTask.taskDependencies.getDependencies(buildTask).collect { it.path }.sort() == ([':child1:jar', ':child1:testJar'])
        project.getGradle().getStartParameter().getTaskNames() == ['eclipseClosedProjectBuildDependencies']
    }
    ...
    def "build task is not added if no projects are closed"() {
        ...
        then:
        project.tasks.findByName("eclipseClosedProjectBuildDependencies") == null
    ...
    def "task name is deduplicated"() {
        setup:
        def eclipseRuntime = eclipseRuntime([gradleProject("child1", false), gradleProject("child2")])
        project.tasks.create("eclipseClosedProjectBuildDependencies")
        def modelBuilder = createBuilder()
        ...
        then:
        def buildTask = project.tasks.getByName("eclipseClosedProjectBuildDependencies_")
        buildTask != null
        buildTask.taskDependencies.getDependencies(buildTask).collect { it.path }.sort() == ([':child1:jar', ':child1:testJar'])
        project.getGradle().getStartParameter().getTaskNames() == ['eclipseClosedProjectBuildDependencies_']
    }
